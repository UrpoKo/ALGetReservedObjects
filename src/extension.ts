...
export function activate(context: vscode.ExtensionContext) {
	...
	console.log("UrpoK's AL 'GetReservedObjects' is now active!");
	...
	let disposable = vscode.commands.registerCommand('getreservedobjects.getReservedObjects', async function () {
		...
		async function loopFiles(folder: vscode.Uri): Promise<{ outlines:string }> {
			...
			for (const [name, type] of await vscode.workspace.fs.readDirectory(folder)) {
				...
				if (type === vscode.FileType.File) if (name.endsWith('.al')) {
					//console.log("Found an .al file '%s'", flatFilePath);
					try {
		...
		if (info.outlines) {
			...
			var tmpArray1 = tArray.concat(pArray,cArray,mArray,qArray,eArray,rArray,xArray).filter(Boolean).sort()
			var tmpArray2 = peArray.concat(teArray,reArray,eeArray).filter(Boolean).sort()
		...
		} else console.error("No .al files containing object reservations found from '%s' !", folderUri.path.substring(1));

		//await vscode.workspace.fs.writeFile(fileUri2, writeData2);
